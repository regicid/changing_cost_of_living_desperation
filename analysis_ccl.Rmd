---
title: "Poverty is associated with both risk avoidance and risk taking: a preregistered empirical test
  of the desperation threshold model."
output:
  word_document:
...
```{r, echo=FALSE, fig.width=25, fig.asp=.25, fig.align="default", fig.cap="Figure 1: Desperation threshold utility function (A) and resulting predicted relationship between resource and risk taking, with perfect observation (B) and with noisy observation, with larger noise on resources (C)"}
...
d$risk_consistent = d$risk_1<=d$risk_7
...
-   
    ...
    (i) whether variance in risk taking was higher among individuals reporting that "managing financially is very difficult";

-   

    (ii) whether squared residuals of a linear model were higher at the bottom of the resource distribution; and 

-   

    (iii) whether participants with lower resources were less stable over time in their risk taking.
...
```{r echo=FALSE,fig.show="hold", out.width="50%",fig.asp=1,message=FALSE,warning = FALSE,fig.cap = "Figure 3: Risk taking predictions by the nonlinear statistical models, depending on objective resources (A, C) and subjective resources (B, D), using cubic regressions (A, B) and and segmented regressions (C, D). The resource range covers 99% of the distribution, excluding the top and bottom 0.5%."}
...
```{r echo=FALSE,message=FALSE,warning=FALSE,fig.cap = 'Figure 4: variance in risk taking among participants, grouped by their answer in the "managing financially" question. The error bars represent a 50% confidence interval for the estimation of the variance, which corresponds approximately to ±1 standard error in the case of a mean.'}
p_values = list()
effect = list()
...
```{r, echo=FALSE,fig.cap="Figure 4: Ratio of variances in risk taking below and above resource thresholds set at different levels."}
...
t_main_text = set_caption(t_main_text,"Table 4: Extreme risk taking prevalence among low-resources categories",align_with_table = F)
...
t = flextable(results)
t = set_caption(t,"Table S2: risk taking statistics by resources categories",align_with_table = F)
...
t_time_discounting = set_caption(t_time_discounting,"Table S3: Time discounting statistics by disadvantaged categories",align_with_table = F)
...
The authors report no conflict of interest. WEF’s contributions have been supported by the Dutch Research Council (V1.Vidi.195.130) and the James S. McDonnell Foundation (https://www.jsmf.org/grants/20170007/). DN’s contribution was supported by ANR grant ANR-21-CE28-0009. The authors thank Ulysse Klatzmann, Camille Boissel and Aurélien Allard for their precious help.
...
```{r fig_descr, echo=FALSE, fig.cap="Figure S1: mean and variance in risk taking for participants, grouped by their answer in the 'managing financially' question", warning=FALSE}
...
```{r echo=FALSE,message=FALSE,warning = FALSE, fig.show="hold", out.width="50%",fig.cap="Figure S2: Deviance of the statistical models depending on the changepoint location, using objective (A) and subjective (B) resources"}
...
table = set_caption(table,"Table S1: Standardised coefficients of the model using subjective resources as independent variable, and the alternative changepoint.",align_with_table = F)
